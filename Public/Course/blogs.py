...
try:
    ...
    if(jk=='inst'):
        ...
        q5=f.getvalue('b5')
        q6=f.getvalue('b6')
        url="insert into blogs(headline,msg,image,author,pub_date,u_id) values(%s,%s,%s,%s,%s,%s)"
        x.execute(url,(q1,q3,q5,q4,q2,q6))
        con.commit()
        print("Successfully Inserted&&0")
    # elif(jk=='inst_car'):
    #     q1=f.getvalue('d1')
    #     q2=f.getvalue('d2')
    #     q3=f.getvalue('d3')
    ...
    #     print("Successfully Deleted!")
        ...
        url="select sn,image,headline from blogs order by sn desc limit 4"
        x.execute(url)
        rs=x.fetchall()
        if(rs!=[]):
            for a in rs:
                ...
                # print('&&&&&*****')
                # # print(a+'          ')
                print('<div class="items" style="margin-left:2rem;"><a href=""><div class="item-img"><img src="'+a[1].decode()+'" alt=""></div></a><p style="overflow:hidden;height:32%;border:solid 1px red;">'+a[2]+'</p></div>')
        ...
        if(rs!=[]):
            ...
            print("10")
    elif(jk=='course_search'):
        x.execute('select sn,headline,image from blogs order by sn desc limit 15')
        rs=x.fetchall()
        if(rs!=[]):
            print('<h3 style="text-align: center;">Recent Blogs</h3>')
            for a in rs:
                print('<div class="pst"><img src="'+a[2].decode()+'" alt="Blog Image">&nbsp;&nbsp;&nbsp;<a style="overflow-x:hidden;">'+str(a[1])+'</a><span style="display:none;">'+str(a[0])+'</span></div><hr>')
    elif(jk=='blogsn'):
        t1=f.getvalue('t1')
        t2=f.getvalue('t2')
        ...
        if(rs!=[]):
            ...
            if(rs!=[]):
                for a in rs:
                    print('<div id="bot_dv"><div id="img1"><img src="'+a[2].decode()+'" alt="" style="width: 100%;height: 100%;"></div><hr><span id="l">'+a[3]+'</span><hr><span id="dt">'+str(a[1])+'</span><span style="color:red;" hidden id="sn">'+str(a[0])+'</span></div>')
                    # print('<div id="bot_dv"><div id="img1"><img src="" alt="" style="width: 100%;height: 100%;"></div><hr><span id="l"></span><hr><span id="dt"></span><span style="color:red;" hidden id="sn">1</span></div>')
                print('</div><button onclick="topFunction()" id="myBtn" title="Go to top"><i class="fas fa-arrow-alt-circle-up"></i></button>')
    else:
        sn=f.getvalue('t1')
        ...
        if(rs!=[]):
            for a in rs:
                print('<div class="container my-5" id="topic" style="font-size: 3rem;text-align: center;font-family:"Gill Sans", "Gill Sans MT", Calibri, "Trebuchet MS", sans-serif;"><p>'+str(a[1])+'</p></div><div id="img"><img src="'+a[3].decode()+'" alt="Blog Image" style="width: 100%;height: 100%;border-radius: 0.5rem;"></div><div class="container-fluid" id="desc" style="font-size: 2rem;padding: 5rem;line-height: 3rem;"><p style="text-align: justify;">'+str(a[2])+'</p></div>')
            x.execute("select sn,pub_date,image,headline from blogs where pub_date<='"+str(rs[0][5])+"' and sn not in("+str(rs[0][0])+") order by sn desc limit 3;")
            rs=x.fetchall()
            if(rs!=[]):
                print('<h1 style="text-align: center;font-weight: bolder;">Recommended Blogs</h1><hr><div class="conta">')
                for a in rs:
                    print('<div id="bot_dv"><div id="img1"><img src="'+a[2].decode()+'" alt="" style="width: 100%;height: 100%;"></div><hr><span id="l">'+str(a[3])+'</span><hr><span id="dt">'+str(a[1])+'</span><span style="color:red;" hidden id="sn">'+str(a[0])+'</span></div>')
                print('</div><button onclick="topFunction()" id="myBtn" title="Go to top"><i class="fas fa-arrow-alt-circle-up"></i></button>')
            else:
                x.execute("select sn,pub_date,image,headline from blogs where pub_date<=current_date() and sn not in("+str(rs[0][0])+") order by sn desc limit 3;")
                rs=x.fetchall()
                if(rs!=[]):
                    print('<h1 style="text-align: center;font-weight: bolder;">Recommended Blogs</h1><hr><div class="conta">')
                    for a in rs:
