    ...
    if d1=='search':
        ...
        for i in rs:
            ...
            if c>=0:
                ...
                print('<div id="d2"><p style="text-align: center;"><input type="button" value="Apply Now" id="b1"></p><p><i class="fas fa-users" id="iright"></i> &nbsp; <label>'+i[4]+'</label> vacancies</p><p><i class="fas fa-cubes"></i> &nbsp; <label>'+i[3]+'</label> Years Experience</p><p><i class="fa fa-map-marker"></i> &nbsp; &nbsp; Place : <label>'+i[6]+'</label></p><p><i class="fas fa-donate"></i>&nbsp;  &nbsp; Salary : <label>'+i[7]+'</label></p><p style="text-align: center;"><i class="fas fa-check" id="status" style="color: blue;"></i> &nbsp; <label style="color: #07ac93 ;">Active</label></p></div></div>')
        for i in rs:
            x=i[5].split('-')
            b=date(int(x[0]),int(x[1]),int(x[2]))
            c=(b-a).days
            if c<0:
                ...
                print('<div id="d2"><p style="text-align: center;"><br><p><i class="fas fa-users" id="iright"></i> &nbsp; <label>'+i[4]+'</label> vacancies</p><p><i class="fas fa-cubes"></i> &nbsp; <label>'+i[3]+'</label> Years Experience</p><p><i class="fa fa-map-marker"></i> &nbsp;  &nbsp; Place : <label>'+i[6]+'</label></p><p><i class="fas fa-donate"></i>&nbsp;  &nbsp; Salary : <label>'+i[7]+'</label></p><p style="text-align: center; color:red;">X &nbsp; <label>Inactive</label></p></div></div>')
    elif d1=='job_apply':
        ...
        if dd=='fl':
            ...
            d6=f.getvalue('t6')
            d7=int(f.getvalue('t7'))
            url='select * from internship where stipend>=%d'%(d7)
            if d2!=None:
            ...
            t.execute('select * from internship order by sn desc')
        rs=t.fetchall()
        from datetime import date
        a=date.today()
        if(rs!=[]):
